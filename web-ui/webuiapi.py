...
# sample -> sampling method 扩散去噪算法的采样模式
# euler a 30步左右即可,后续的调整多为降噪为主,LMS需要大约70步才能得出最好结果,DDIM会用更写实的光影和色彩，PLMS介于两者之间
...
class WebUIApi:
    ...
    def txt2img(self, 
        ...
        payload = {    
            "enable_hr": enable_hr, # enable_hr is the toggle for hires fix
            ...
            "n_iter": n_iter,
            "steps": steps,
            "cfg_scale": cfg_scale, # 分类器自由引导尺度,越大越严格
            "width": width,
            "height": height,
            "restore_faces": restore_faces, # 面部优化
            "tiling": tiling, # 生成一个可以平铺的图像
